...
class HG2eUCN:
    ...
    def get_typicals(self, proj) -> pd.DataFrame:
        ...
        def func_unique(row):
            curTypical = row["Typical"]
            while curTypical in mylist:
                curTypical += "@"
            mylist.append(curTypical)
            return curTypical

        try:
            print("- Loading Typicals...")
            typicals = pd.read_excel(self.rulespath + "EPLCG2SMM_typicals.xlsx")
            typicals["OrigTypical"] = typicals["Typical"]
            typicals["Typical"] = typicals.apply(lambda row: func_unique(row), axis=1)
            return typicals
        except FileNotFoundError:
            print(
                f"{ERROR}ERROR: file "
                f"'EPLCG2SMM_typicals.xlsx' not found in '{self.rulespath}'"
            )
            exit(f"ABORTED: File not found{RESET}")

    def read_typicals(self, proj, phase):

        # !! Typicals will only be assigned first time, after that manual changes can be done.
        # !! To clear typicals, delete the file {proj}_Typical_assignment_{phase1}.xlsx

        dprint(f"- Loading or assigning typicals...", "CYAN")
        self.typ = self.get_typicals(proj)
        self.typ = self.typ.rename(columns={"PNTTYPE": "&T"})
        if not file_exists(self.projpath + f"{proj}_Typicals_{phase}.xlsx"):
            dprint("No typical file found, creating new one", "GREEN")

            self.names, self.typicals = self.assign_typicals()
            quick_excel(
                self.typicals,
                self.projpath,
                f"{proj}_Typicals_{phase}",
                False,
            )
        else:
            dprint("Existing typical file found, using that one", "GREEN")
            self.typicals = read_db(self.projpath, f"{proj}_Typicals_{phase}.xlsx")
            self.names = self.typicals[["&N", "Typical"]].copy(deep=True)
    ...
    def get_rules(self, proj) -> pd.DataFrame:
        try:
            ...
            df_temp = pd.read_excel(
                self.rulespath + "EPLCG2SMM_rules.xlsx",
            )
    ...
    def Dig_rules(self, proj, sheet) -> typing.Any:
        try:
            print("- Loading Rules...")
            df_temp = pd.read_excel(
                self.rulespath + "EPLCG2SMM_tables.xlsx",
                sheet_name=sheet,
            )
            ...
            print(
                f"{ERROR}ERROR: file "
                f"'EPLCG2SMM_tables.xlsx' not found in "
                f"'{self.rulespath}'"
            )
            exit(f"ABORTED: File not found{RESET}")
    ...
    def get_eb_files(self, proj, phase) -> typing.Dict[str, pd.DataFrame]:
        def add_columns(df, sheet, COM):
            if sheet.upper() != "TOTAL":
                for column in self.all_columns[COM][sheet]:
    ...
    def get_PLC_match(self, proj, phase) -> pd.DataFrame:
        try:
            ...
            return pd.read_excel(
                self.projpath + f"{proj}_PLCmatch_{phase}.xlsx",
            )
        except FileNotFoundError:
            print(
                f"{ERROR}ERROR: file "
    ...
    def assign_typicals(self):
        ...
        if names[names["Typical_count"] != 1].size > 0:
            ...
            exit(
                f"{Fore.RED}Aborted - none or non-single typical assignment - check 'DEBUG.xlsx in rules folder'{Fore.RESET}"
            )
        names["Typical"] = ""
        ...
        names = names.merge(
            self.typ[["Typical", "OrigTypical"]], how="inner", on="Typical"
        )

        names = names.rename(
            columns={"Typical": "OrigTypical", "OrigTypical": "Typical"}
        )
    ...
    def link1_1(self, typical, index, row):
        def func_check(row, param):
            ...
            if row[param] == param_old:
                ...
                if param in self.warnings:
                    return f"warning: was {str(param_old)}"
                else:
    ...
    def link1_1_round_TP(self, typical, index, row):
        def func_check(row, param: str):
            ...
            PVLO_EU = float(row["PVEULO_old"])
            PVHI_EU = float(row["PVEUHI_old"])
            param_old = float(row[f"{param}_old"])
            max_range = max(
                abs(float(row["PVEUHI_old"])), abs(float(row["PVEULO_old"]))
            )
            ...
            if decimals == 0:
                ...
                PVHI_EU = int(round(PVHI_EU, 0))
                PVLO_EU = int(round(PVLO_EU, 0))
            else:
                rounded_old = round(param_old, decimals)
                rounded_EU = round(float(row[f"{param_EU}_old"]), decimals)
                PVHI_EU = round(PVHI_EU, decimals)
                PVLO_EU = round(PVLO_EU, decimals)
            if "LO" in param:
            ...
            if "HI" in param:
                if rounded_old >= rounded_EU or rounded_old == PVLO_EU:
                    if row[param] == "--------":
                        return "OK"
                    else:
                        return f"'-----' expected ({param} outside range - {param_EU}: {rounded_EU})"
    ...
    def checkDO_rules(self, typical):
        def func_check(row, param):
            ...
            try:
                ...
                pass
            if row[param] == param_rules or (
                str(row[param])[:3] == "---" and str(param_rules) == ""
            ):
                return "OK"
            else:
    ...
    def check_all(self):
        ...
        for index, row in self.typ.iterrows():
            if "IGNORE" not in str(row["OrigTypical"]):
                typical = row["OrigTypical"]
                if typical not in done:
                    done.append(typical)
    ...
    def main_def(self, proj, phase0, phase1) -> typing.Any:
        ...
        newPLCs = my_proj.PLCs[phase1]
        self.rulespath = f"Projects\\Templates\\"
        ...
        # & Create a typical file, or use the existing one (manually changable)
        self.read_typicals(proj, phase1)
        ...
        self.warnings = ["PRIMMOD", "PVTV", "$REALARM", "NMODATTR", "ASSOCDSP"]
        ...
        if output == 1 or output == 2:
            quick_excel(self.main, self.projpath, "quick_check")

        if output == 0 or output == 2:
            filename = f"{proj}_EB_check_{phase1}.xlsx"
    ...
    def start(self, project, phase0, phase1):
        ...
        print(
            f"{Fore.MAGENTA}Comparing HG to eUCN for {Fore.GREEN}{project}{Fore.MAGENTA}, phase {Fore.GREEN}{phase1}{Fore.RESET}"
        )

        self.main_def(project, phase0, phase1)
        print(
            f"{Fore.MAGENTA}Finished Comparing HG to eUCN for {Fore.GREEN}{project}{Fore.MAGENTA}, phase {Fore.GREEN}{phase1}{Fore.RESET}"
        )


# TODO add rules 98/99 checks
# TODO add typicals to NEW tags
# TODO EIP check
# TODO pre-filter EB-files
# Done fix when column is missing (e.g. PVLOPR SWS6)


def main():
    system("cls")
    project = HG2eUCN("PSU30_35_45", "Original", "Final")
