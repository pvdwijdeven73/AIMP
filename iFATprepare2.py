...
from routines import format_excel, show_df, ProjDetails, read_db, dprint  # ,ErrorLog
import json
...
class iFATprepare:
    ...
    def compile_dcs(self, db_dcs: Any, plc_list: list) -> pd.DataFrame:
        ...
        all_params = hwy_params + ucn_params
        for param in all_params:
            if param not in db_dcs.columns:
                dprint(f"Empty column {param} added to DCS database", "GREEN")
                db_dcs[param] = ""
    ...
    def start(self, project):
        ...
        outputFile = f"{project}_preparation.xlsx"
        ...
        HMIExport = f"{project}_HMI.xls"  # - placed in path+"Exports\\"
        CLExport = f"{project}_CLrefs.xls"  # - placed in path+"Exports\\"
